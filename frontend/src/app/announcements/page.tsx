        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs text-gray-400">• School-wide Bulletin</span>
            </div>
          ...
          {announcements.map(a => (
            <div
              ...
              className={`glass-panel p-6 rounded-2xl border space-y-3 transition-all ${
                a.pinned ? 'border-indigo-500/50 bg-indigo-950/10' : 'border-gray-800 hover:border-gray-700'
              }`}
            >
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                ...
                <span className="text-xs text-gray-400">Posted by <span className="text-gray-200 font-medium">{a.author}</span></span>
              </div>

              <h2 className="text-base font-bold text-white">{a.title}</h2>
              <p className="text-xs text-gray-300 leading-relaxed bg-gray-950/40 p-3.5 rounded-xl border border-gray-800/80">
                {a.content}
              </p>
            </div>
          ))}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in">
            <div className="glass-panel border border-gray-700 max-w-md w-full rounded-2xl p-6 space-y-4 shadow-2xl">
              <div className="flex items-center justify-between border-b border-gray-800 pb-3">
                <h3 className="text-base font-bold text-white">Broadcast School Circular</h3>
              ...
              <form onSubmit={handleCreate} className="space-y-3 text-xs">
                <div>
                  <label className="text-gray-300 font-semibold block mb-1">Circular Subject</label>
                  <input
                    type="text"
                    value={newAnn.title}
                ...
                <div>
                  <label className="text-gray-300 font-semibold block mb-1">Circular Content</label>
                  <textarea
                    rows={4}
