...
export default function LibrarianDigital() {
  ...
  const fetchResources = async () => {
    try {
      const res = await api.get("/librarian/digital");
            ...
            <table className="w-full text-left text-sm text-gray-600 min-w-max">
              ...
              <tbody className="divide-y divide-gray-800">
                {filteredResources.map((res) => (
                  <tr key={res.id} className="hover:bg-gray-100/30 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-bold text-brand-black">{res.title}</div>
                      <div className="text-xs text-gray-500">Added: {new Date(res.created_at).toLocaleDateString()}</div>
                    </td>
                    <td className="px-6 py-4 font-medium text-violet-600">{res.category}</td>
                    <td className="px-6 py-4 text-center font-bold text-brand-black">{res.access_count || 0}</td>
                    ...
                    <td className="px-6 py-4">
                      <a href={res.url} target="_blank" rel="noreferrer" className="text-gray-600 hover:text-brand-black flex items-center gap-2 text-xs font-bold transition-colors">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-2xl w-full max-w-md overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <input value={title} onChange={e => setTitle(e.target.value)} type="text" className="w-full bg-gray-100 border border-gray-200 text-brand-black rounded-lg px-4 py-2 focus:outline-none focus:border-violet-500" placeholder="e.g. Science Journal" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-600 mb-1">Resource URL</label>
                  <input value={url} onChange={e => setUrl(e.target.value)} type="url" className="w-full bg-gray-100 border border-gray-200 text-brand-black rounded-lg px-4 py-2 focus:outline-none focus:border-violet-500" placeholder="https://" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-600 mb-1">Category / Type</label>
                  <select value={category} onChange={e => setCategory(e.target.value)} className="w-full bg-gray-100 border border-gray-200 text-brand-black rounded-lg px-4 py-2 focus:outline-none focus:border-violet-500">
                    <option>E-Book</option>
                    <option>Journal</option>
                    <option>Video Course</option>
              ...
              <div className="p-6 border-t border-gray-200 bg-gray-50/50 flex justify-end gap-3">
                ...
                <button onClick={handleSave} className="bg-violet-600 hover:bg-violet-700 text-brand-black px-4 py-2 rounded-lg transition-colors">Add Resource</button>
              </div>
            </div>
          </div>
        )}
