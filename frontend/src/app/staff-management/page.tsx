    ...
    <ProtectedRoute allowedRoles={["admin", "principal", "super_admin", "correspondent"]}>
      <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs text-gray-400">• Staff Hub</span>
            </div>
        ...
        <div className="w-full overflow-x-auto pb-2 -mb-2">
          <div className="inline-flex rounded-xl bg-gray-900/80 p-1 border border-gray-800 min-w-max">
            <button
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="glass-panel p-4 rounded-2xl border border-gray-800 space-y-1">
                <div className="text-xs text-gray-400">Present on Campus</div>
                <div className="text-2xl font-bold text-emerald-400">65 Faculty</div>
                <div className="text-[11px] text-gray-400">95.6% attendance compliance today</div>
              </div>
              <div className="glass-panel p-4 rounded-2xl border border-gray-800 space-y-1">
                <div className="text-xs text-gray-400">Approved Leave / Off-Duty</div>
                <div className="text-2xl font-bold text-amber-400">3 Faculty</div>
                <div className="text-[11px] text-gray-400">Covered by substitute teachers</div>
              </div>
              <div className="glass-panel p-4 rounded-2xl border border-gray-800 space-y-1">
                <div className="text-xs text-gray-400">Late Punch-ins</div>
                <div className="text-2xl font-bold text-cyan-400">1 Faculty</div>
                <div className="text-[11px] text-gray-400">Marked before morning assembly</div>
              </div>
            </div>

            <div className="glass-panel p-5 rounded-2xl border border-gray-800 space-y-4">
              <h2 className="text-base font-bold text-white">Daily Faculty Attendance Ledger</h2>
                  ...
                  <tbody className="divide-y divide-gray-800/60">
                    {staffList.map(s => (
                      <tr key={s.id} className="hover:bg-gray-900/40 transition-colors">
                        ...
                        <td className="p-3.5 text-gray-300">{s.department}</td>
                        <td className="p-3.5 font-mono text-gray-300">{s.checkin}</td>
        ...
        {activeTab === "council" && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {meetings.map(m => (
                <div key={m.id} className="glass-panel p-5 rounded-2xl border border-gray-800 space-y-3 flex flex-col justify-between">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-gray-400">Venue: <span className="text-gray-200">{m.venue}</span></p>
                  </div>
            ...
            <div className="glass-panel p-4 rounded-2xl border border-gray-800 flex items-center justify-between">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-8 pr-3 py-1.5 rounded-xl bg-gray-900 border border-gray-700 text-white text-xs w-64"
                />
              </div>
              <span className="text-xs text-gray-400 font-mono">{filteredStaff.length} Faculty Members</span>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredStaff.map(s => (
                <div key={s.id} className="glass-panel p-5 rounded-2xl border border-gray-800 space-y-3">
                  <div className="flex items-center gap-3 border-b border-gray-800 pb-3">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-indigo-600 to-cyan-400 flex items-center justify-center font-bold text-white">
                  ...
                  <div className="space-y-1.5 text-xs text-gray-300">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-gray-400">{s.email}</span>
                    </div>
                    <div className="flex items-center gap-2">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in">
            <div className="glass-panel border border-gray-700 max-w-md w-full rounded-2xl p-6 space-y-4 shadow-2xl">
              <div className="flex items-center justify-between border-b border-gray-800 pb-3">
                <h3 className="text-base font-bold text-white">Schedule Staff Council Meeting</h3>
                <button onClick={() => setShowMeetingModal(false)} className="text-gray-400 hover:text-white">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={handleCreateMeeting} className="space-y-3 text-xs">
                <div>
                  <label className="text-gray-300 font-semibold block mb-1">Meeting Agenda / Title</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-gray-300 font-semibold block mb-1">Date & Time</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-gray-300 font-semibold block mb-1">Campus Venue</label>
                  <input
                    type="text"
