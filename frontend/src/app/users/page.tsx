...
function UsersPageContent() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-8 gap-3">
        {ALL_ROLES.map(role => (
          <button
            ...
            className={`bg-white rounded-[24px] border border-gray-100 shadow-sm p-3 rounded-xl text-center transition-all cursor-pointer
              ${roleFilter === role ? 'border-indigo-500/50 bg-brand-blue/10' : 'hover:border-gray-600'}`}
          >
            <div className="text-lg font-bold text-brand-black">{roleCounts[role] || 0}</div>
            <div className="text-[10px] text-gray-600 mt-0.5">{ROLE_LABELS[role]}</div>
          </button>
        ))}
      </div>
      ...
      {/* Users Table */}
      <div className="bg-white rounded-[24px] border border-gray-100 shadow-sm rounded-2xl overflow-hidden">
                    ...
                    <tr key={user.id} className="hover:bg-gray-100/20 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-medium text-brand-black">{user.full_name}</div>
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full ${
                          user.is_active ? 'bg-emerald-500/10 text-emerald-600 border border-emerald-500/30' : 'bg-red-500/10 text-red-400 border border-red-500/30'
                        }`}>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[24px] border border-gray-100 shadow-sm-glow rounded-2xl p-6 w-full max-w-lg space-y-5">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold text-brand-black">Create New User</h3>
