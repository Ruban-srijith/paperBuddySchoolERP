    ...
    <ProtectedRoute allowedRoles={['warden', 'super_admin', 'admin', 'principal']}>
      <div className="space-y-6 max-w-3xl mx-auto">
        <header className="mb-8">
          ...
          <p className="text-gray-400 mt-2">Request funds for hostel maintenance, food supplies, repairs, and utilities.</p>
        </header>
        ...
        <form onSubmit={handleSubmit} className="glass-panel p-8 rounded-2xl border border-gray-800 space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-400 mb-2">Urgency & Justification</label>
            <textarea 
              value={description} 
              onChange={e => setDescription(e.target.value)}
              placeholder="Explain why these funds are needed immediately..."
              className="w-full bg-gray-900/50 border border-gray-700 text-white rounded-xl px-4 py-3 h-32 focus:outline-none focus:border-amber-500" 
              required 
            />
          </div>
          <button type="submit" className="w-full bg-amber-600 hover:bg-amber-700 text-white font-bold py-4 rounded-xl flex justify-center items-center gap-2">
